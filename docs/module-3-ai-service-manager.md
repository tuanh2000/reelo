...
| **Subscription-CLI** | Shell ra `claude` CLI headless bằng token subscription của user (BYO) | **claude-cli** write-script (CLI Claude Code) |
...
>
> **Subscription-CLI provider `claude-cli` — CLAUDE QUA SUBSCRIPTION (BYO):** `clients/claude_cli.py::ClaudeCliClient` (`WRITE_SCRIPT`, `cost_tier=free` — Reelo KHÔNG bill token, dùng subscription của user, `requires_key=True`, key_ref `claude_oauth`, `provider_id="claude-cli"`). KHÁC provider `claude` (Anthropic API key trả-theo-token): mỗi user dùng **tài khoản Claude của CHÍNH HỌ** — tạo OAuth token bằng `claude setup-token` (đăng nhập subscription), dán vào Setup; Reelo lưu **mã hóa per-user** (KeyStore, key_ref `claude_oauth`) và `asyncio.create_subprocess_exec` ra `claude` CLI **headless** với token đó. **KHÔNG** dùng `run_skill_script` (parse JSON skill-format) — exec trực tiếp + parse output CLI native. **Argv** (verify CLI 2.1.x): `claude -p "<prompt>" --output-format json --model <map> --tools "" --max-turns 1 --permission-mode default --no-session-persistence [--system-prompt <req.system>]` → 1 lượt, không tool/agentic. **Auth headless**: env `CLAUDE_CODE_OAUTH_TOKEN=<token>` (bearer) + `CLAUDE_CONFIG_DIR` = thư mục temp riêng/call để token user không đụng keychain/session của nhau (và token sai fail sạch `api_error_status:401` thay vì rơi về tài khoản máy đang đăng nhập); cũng set `ANTHROPIC_API_KEY=""` để API key máy không che token. **Parse**: stdout là 1 object JSON `{type:"result", is_error, result:"<text>", usage:{input_tokens,output_tokens}, modelUsage}`; lấy `result` làm text. **Lỗi**: exit≠0 hoặc `is_error` + 401/403/"Not logged in"/"Invalid bearer token" → `InvalidKeyError` (không fallback); lỗi khác/timeout/CLI thiếu → `ProviderUnavailableError`. Map model: `claude-opus-4-8` / `claude-sonnet-4-6` (default) / `claude-haiku-4-5`. **json_schema**: CLI có `--json-schema` nhưng để đồng nhất cơ chế RULE/sentinel (Module 1 parse JSON từ text), client chỉ thêm gợi ý "trả JSON" vào prompt và trả text — Module 1 parse phần còn lại (`supports_json_mode: false`). `validate_key` = 1 call `claude -p "ok"` với token (exit 0 = hợp lệ). `is_available` = có token. **KHÔNG ở `routing.fallback`** — chọn explicit ở Setup (cần token subscription đặc thù). **Hạ tầng**: worker image cài Node.js + `@anthropic-ai/claude-code` (`npm i -g`) → image to hơn, chấp nhận. `REELO_CLAUDE_BIN` override path binary (tests/alt).
>
> **⚠️ Caveat ToS (claude-cli):** đây là **BYO subscription token** — mỗi user dùng tài khoản Claude của **chính họ**; **Reelo KHÔNG cung cấp Claude** và token subscription chỉ phục vụ **chính chủ** (không share/đa người dùng chung 1 token — vi phạm ToS Anthropic). Token do user tự tạo (`claude setup-token`), lưu mã hóa per-user, bơm env just-in-time, **không log**. Hạn chế: chịu **rate-limit của plan subscription** user (Pro/Max…); token **có thể hết hạn** → call trả `InvalidKeyError`, user tạo token mới ở Setup; image worker **lớn hơn** vì đóng gói Node + CLI.
